...
def save_checkpoint(epoch, name, model, light, discr1, discr2, optimizer, optimizer_D1, optimizer_D2, output_dir, filename, is_best=False):
    model_state = model.module.state_dict() if is_parallel(model) else model.state_dict()
    light_state = light.module.state_dict() if is_parallel(model) else light.state_dict()
    discr1_state = discr1.module.state_dict() if is_parallel(discr1) else discr1.state_dict()
    discr2_state = discr2.module.state_dict() if is_parallel(discr2) else discr2.state_dict()
    checkpoint = {
            'epoch': epoch,
            'model': name,
            'state_dict': model_state,
            'light_dict': light_state,
            'discr1_dict': discr1_state,
            'discr2_dict': discr2_state,
