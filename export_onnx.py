...
if __name__ == "__main__":
    ...
    modelmulti = MCnet(YOLOP)
    checkpoint = torch.load("./weights/light_multi.pth", map_location=device)
    modelmulti.load_state_dict(checkpoint['state_dict'])
    modelmulti.eval()

    lightnet = relight(3, 64, mode='sobel')
    ...
    height = args.height
    width = args.width
    onnx_path = f'./weights/lightmulti-{height}-{width}.onnx'
    inputs = torch.randn(1, 3, height, width)

    print(f"Converting to {onnx_path}")
    torch.onnx.export(modelmulti, inputs, onnx_path,
