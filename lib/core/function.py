...
def multidetach(multipred):
    pred= [[None, None, None], None, None]
    for i in range(len(multipred[0])):
        pred[0][i]= multipred[0][i].float().detach()
    pred[1]= multipred[1].float().detach()
    pred[2]= multipred[2].float().detach()
    return pred


def train(cfg, train_loader, targetloader_iter, model, lightnet, discr1, discr2, criterion, loss_exp_z, loss_TV, loss_SSIM, loss_pair,
              optimizer, optimizer_D1, optimizer_D2, scaler, epoch, num_batch, num_warmup, writer_dict, logger, device, rank=-1):
    """
    train for one epoch
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        _, combine= targetloader_iter.__next__()
        optimizer.zero_grad()
        optimizer_D1.zero_grad()
        optimizer_D2.zero_grad()

        '''train relight+multinet'''
        for param in discr1.parameters():
            param.requires_grad = False
        for param in discr2.parameters():
            param.requires_grad = False

        # 1. train with target
        input_n, input_d, _, _ = combine
        input_d= input_d.to(device)

        mean_light = input_n.mean()
        r = lightnet(input_d)
        enhanced_images_d= input_d+ r
        loss_enhance = 10* loss_TV(r) + torch.mean(loss_SSIM(enhanced_images_d, input_d)) \
                       + torch.mean(loss_exp_z(enhanced_images_d, mean_light))

        pred_target_d = model(enhanced_images_d)
        D_out_d = discr1(pred_target_d)
        D_label_d = torch.zeros_like(D_out_d.data).fill_(source_label).to(device)
        ...
        r = lightnet(input_n)
        enhanced_images_n = input_n + r
        loss_enhance = 10 * loss_TV(r) + torch.mean(loss_SSIM(enhanced_images_n, input_n)) \
                       + torch.mean(loss_exp_z(enhanced_images_n, mean_light))

        pred_target_n = model(enhanced_images_n)
        pred_target_d_seg= [None, pred_target_d[1].data, pred_target_d[2].data]

        D_out_n = discr2(pred_target_n)
        D_label_n = torch.zeros_like(D_out_n.data).fill_(source_label).to(device)
        loss_adv_target_n = weightedMSE(D_out_n, D_label_n)

        loss_day_night = loss_pair(pred_target_d_seg, pred_target_n, shapes)
        loss = 0.01 * loss_adv_target_n + loss_day_night + 0.01 * loss_enhance
        scaler.scale(loss).backward()
        ...
        r = lightnet(input)
        enhanced_images = input + r
        loss_enhance = 10 * loss_TV(r) + torch.mean(loss_SSIM(enhanced_images, input)) \
                       + torch.mean(loss_exp_z(enhanced_images, mean_light))
        with amp.autocast(enabled=device.type != 'cpu'):
            pred_source = model(input)
            loss_total, losses_head = criterion(pred_source, target, shapes, model)
        loss= loss_total+ 0.01* loss_enhance
        ...
        # 1. train with source
        pred_source= multidetach(pred_source)
        D_out1 = discr1(pred_source)
        D_label1 = torch.zeros_like(D_out1).fill_(source_label).to(device)
        loss_D1 = weightedMSE(D_out1, D_label1)
        scaler.scale(loss_D1).backward()

        pred_source= multidetach(pred_source)
        D_out2 = discr2(pred_source)
        D_label2 = torch.zeros_like(D_out2).fill_(source_label).to(device)
        loss_D2 = weightedMSE(D_out2, D_label2)
        scaler.scale(loss_D2).backward()

        # 2. train with target
        pred_target_d = multidetach(pred_target_d)
        D_out1 = discr1(pred_target_d)
        D_label1 = torch.zeros_like(D_out1).fill_(target_label).to(device)
        loss_D1 = weightedMSE(D_out1, D_label1)
        scaler.scale(loss_D1).backward()

        pred_target_n = multidetach(pred_target_n)
        D_out2 = discr2(pred_target_n)
        D_label2 = torch.zeros_like(D_out2).fill_(target_label).to(device)
        loss_D2 = weightedMSE(D_out2, D_label2)
        scaler.scale(loss_D2).backward()
...
def validate(epoch, config, val_loader, val_dataset, model, lightnet, criterion, output_dir,
